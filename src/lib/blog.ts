...
/** Map a raw category string → one of the five parenting verticals. */
export function verticalFor(category: string): Vertical {
  const c = category.toLowerCase()
  if (/(wellbeing|health|sleep|screen|feeling|emotion|medic|safety|mental)/.test(c)) return 'health'
  if (/(home|routine|chore|money|budget|meal|organi|space|household)/.test(c)) return 'finance'
  if (/(learn|reading|read|school|book|play|develop|milestone|language|education)/.test(c))
    return 'dev'
  if (/(everyday|culture|family|outing|travel|holiday|tradition|relationship|life)/.test(c))
    return 'culture'
  return 'personal'
}
